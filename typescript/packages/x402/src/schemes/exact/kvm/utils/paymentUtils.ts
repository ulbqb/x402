...
export function encodePayment(payment: PaymentPayload): string {
  ...
  // kvm
  if (SupportedKVMNetworks.includes(payment.network)) {
    safe = {
      ...payment,
      payload: payment.payload as ExactKvmPayload,
    };
    return safeBase64Encode(JSON.stringify(safe));
  }
...
export function decodePayment(payment: string): PaymentPayload {
  ...
  // kvm
  if (SupportedKVMNetworks.includes(parsed.network)) {
    obj = {
      ...parsed,
      payload: parsed.payload as ExactKvmPayload,
    };
  }
